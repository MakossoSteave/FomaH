...
<div class="flex mt-4">
        ...
        <a                             class="has-icons-right" id="link-black">   <!-- href="{{ route('admin.create') }}" -->
    ...
    @if($users->isEmpty())
        <div class="notification is-warning has-text-centered my-4">
            Aucun autre formateur n'existe
        </div>
    @else
                        ...
                        <form                         method="GET">      <!-- action="{{ route('admin.edit', $user->id) }}" -->          
                            ...
                            <div id="{{$user->id}}" class="modal">
                                ...
                                <div class="modal-card">
                                    ...
                                    <p class="modal-card-title">Suppression du formateur {{$user->name}}</p>
                                    <button class="delete" aria-label="close"></button>
                                    </header>
                                    <section class="modal-card-body">
                                        Souhaitez-vous supprimer le formateur {{$user->name}} ?
                                    </section>
                                    <form action="{{ route('destroyParAdmin', $user->id) }}" method="GET">
                                        @csrf
